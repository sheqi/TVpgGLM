...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr  7 19:13:59 2017
...
class NonlinearAutoregressiveModel(ModelGibbsSampling):
    ...
    def generate(self, keep=True, T=100, verbose=False, intvl=10):
        ...
        W = self.weights.reshape((N, T, N*B))  # N x T x N x B
        ...
        for t in range(L,T+L):
            ...
            Psi[t] = W[:,t-L,:].dot(X[t].reshape((N*B,))) + b
    ...
    def plot(self,
        ...
        :return:
        """
        from plotting_tv import plot_glm
        return plot_glm(
...
class _DefaultMixin(object):
    ...
    def __init__(self, T, N, B=10, basis=None,
                 network=None,
                 network_kwargs=None,
                 regressions=None,
        ...
        if network is None:
            ...
            network = self._network_class(T, N, B, **network_kwargs)

        if regressions is None:
            regression_kwargs = dict() if regression_kwargs is None else regression_kwargs
            regressions = [self._regression_class(T, N, B, **regression_kwargs) for _ in range(N)]
        super(_DefaultMixin, self).__init__(N, network, regressions, B=B, basis=basis)
   
class SparseBernoulliGLM_f(_DefaultMixin, NetworkGLM):
    _network_class = networks_tv.FixedMeanDenseNetwork
    _regression_class = regression_tv.SparseBernoulliRegression
